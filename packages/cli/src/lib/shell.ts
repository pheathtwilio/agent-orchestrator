...
export async function execSilent(cmd: string, args: string[], options?: { timeout?: number }): Promise<string | null> {
  try {
    const { stdout } = await exec(cmd, args, { timeout: options?.timeout });
    return stdout;
  } catch {
    return null;
  }
